...
for option in [' random']:
    for texture_name in ['master sizer', 'hydro meter']:
        ...
        cols_for_res_df = ['total loss', 'texture type', "num of features", 'division', 'permutation num', 'features']
        res_dir_name = results_dir + model_name
        res_df_name = res_dir_name + "/res df {0} \n division:{1}.xlsx".format(texture_name, option)
        ...
        for i in range(1, 2**len(cols_for_model)):
            ...
            model_type = RandomForest if model_name == 'random_forest' else LinearReg
