...
def mutate(p1, feature_space):
    ...
    for key in p1:
        if type(p1[key]) == int:
            ...
            new_son[key] = [val if random.random() > threshold else np.random.randint(feature_space[key][0],
                                                                                   feature_space[key][1])
                            for val in p1[key]]
    return new_son


def calc_fitness(p, train_x, val_x, test_x, train_y, val_y, test_y, model_type, test_df, texture_cols):
    # try:
    model = model_type(train_x, val_x, test_x, train_y, val_y, test_y, p)
    model.create_model()
    model.train()
    predictions = model.predict_test()
    res_model = calc_rmses(predictions, test_df, texture_cols)
    total_rmse_index = -1
    return flip_fitness(res_model[total_rmse_index])
...
def print_best_loss(i, best_fit, best_currently):
...
def save_top_5(pop, fitness, model_name, texture_type):
    ...
    if os.path.isfile(file_name):
        ...
        for i in range(min(5, len(fitness))):
            res_dict = {'fitness': [fitness[i]], 'rmse': [flip_fitness(fitness[i])], 'features': [str(pop[i])]}
            row = pd.DataFrame.from_dict(res_dict)
            res_df = pd.concat([res_df, row], sort=True)
...
def iterate(model_type, feature_space, train_x, val_x, test_x, train_y, val_y, test_y, test_df, texture_cols,
    ...
    for i in range(num_iter):
        if new_pop_bol:
            fitness = [calc_fitness(p, train_x, val_x, test_x, train_y, val_y, test_y, model_type, test_df, texture_cols)
                       for p in pop]
        else:
            new_pop_bol = True
        ...
        for _ in range(len(pop) - leave_val):
            ...
            new_pop.append(mutate(rec, feature_space))
        pop = new_pop
    # except Exception as e:
    #     print(e)
    #     print(fitness)
